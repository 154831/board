...
<script type='text/javascript'>
	$().ready(function() {
		// 키 업 할 때 마다, 이메일에 적힌 값을 매번 전달하고, 그에 대한 응답을 function 으로 받는다. 
		$("#email").keyup(function() {
			$.get("/member/regist/available", {
				...
				if (available) {
					...
					$("#btn-regist").removeAttr("disabled")//disabled 라는 속성을 제거해라
				}

				else {
					$("#email").addClass("unusable")
					$("#email").removeClass("available")
					$("#btn-regist").attr("disabled", "disabled")//disabled 라는 속성을 제거해라
				}
			})
		})
	})
